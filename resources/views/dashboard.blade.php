...
<div class="grid grid-cols-3 md:grid-cols-3 grid-rows-4 md:grid-rows-4 gap-2 md:gap-2 m-4">
  ...
  <div class="col-start-1 row-start-4 md:col-start-3 md:row-start-2 md:col-span-1 md:row-span-1 bg-white rounded-md p-6">
  ...
  <div class="col-start-2 row-start-4 md:col-start-3 md:row-start-3 md:col-span-1 md:row-span-1 bg-white rounded-md p-6">
  ...
  <div class="col-start-3 row-start-4 md:col-start-3 md:row-start-4 md:col-span-1 md:row-span-1 bg-white rounded-md p-6">
  ...
  <div class="col-start-1 row-start-2 col-span-3 row-span-2 md:col-start-1 md:row-start-2 md:col-span-2 md:row-span-3 bg-white rounded-md p-6">
    
    <div class="flex justify-between items-start mb-6">
      <p class="font-bold text-lg">MT Pipeline</p>
      <form method="GET" action="{{ route('dashboard') }}" class="flex gap-4 items-start">
        <div>
    ...
    <div class="flex gap-2 items-end w-full h-48">      
      <div class="flex flex-col items-center flex-1">
        <p class="font-bold">{{ $total }}</p>
        <div class="bg-[#FF9A02] w-full" style="height: {{ $total > 0 ? ($total / $total) * 160 : 0 }}px"></div>
        <p class="text-xs mt-2">Hired</p>
      </div>
      <div class="flex flex-col items-center flex-1">
        <p class="font-bold">{{ $graduate }}</p>
        <div class="bg-[#197B40] w-full" style="height: {{ $total > 0 ? ($graduate / $total) * 160 : 0 }}px"></div>
        <p class="text-xs mt-2">Graduate</p>
      </div>
      <div class="flex flex-col items-center flex-1">
        <p class="font-bold">{{ $withdraw }}</p>
        <div class="bg-yellow-400 w-full" style="height: {{ $total > 0 ? ($withdraw / $total) * 160 : 0 }}px"></div>
        <p class="text-xs mt-2">Withdraw</p>
      </div>
      <div class="flex flex-col items-center flex-1">
        <p class="font-bold">{{ $failed }}</p>
        <div class="bg-red-500 w-full" style="height: {{ $total > 0 ? ($failed / $total) * 160 : 0 }}px"></div>
        <p class="text-xs mt-2">Failed</p>
      </div>
      <div class="flex flex-col items-center flex-1">
        <p class="font-bold">{{ $active }}</p>
        <div class="bg-blue-500 w-full" style="height: {{ $total > 0 ? ($active / $total) * 160 : 0 }}px"></div>
